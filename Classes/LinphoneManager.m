...
const int kLinphoneAudioVbrCodecDefaultBitrate = 10; /*you can override this from linphonerc or linphonerc-factory*/
...
#pragma deploymate pop
	...
	if ((state == LinphoneCallEnd || state == LinphoneCallError))
    {
        ...
        NSString *callDicrection = (linphone_call_log_get_dir(callLog) == LinphoneCallIncoming) ? incomming_call : outgoing_call;
        
        NSString *callStatus = @"";
        LinphoneCallStatus eCallStatus = linphone_call_log_get_status(callLog);
        if (eCallStatus == LinphoneCallSuccess) {
            callStatus = success_call;
        }else if (eCallStatus == LinphoneCallAborted){
            callStatus = aborted_call;
        }else if (eCallStatus == LinphoneCallMissed){
            callStatus = missed_call;
        }else if (eCallStatus == LinphoneCallDeclined){
        ...
        NSString *date = [dateFormatter stringFromDate:startData];
        dateFormatter.dateFormat = @"HH:mm:ss";
        NSString *time = [dateFormatter stringFromDate:startData];
...
static int comp_call_id(const LinphoneCall *call, const char *callid) {
    ...
    LinphoneCallLog *callLog = linphone_call_get_call_log(call);
	return strcmp(linphone_call_log_get_call_id(callLog), callid);
}
